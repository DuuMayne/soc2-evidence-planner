...
| 2026-09-15 | **Rebuilt CM.09 patching after finding a timezone defect that inverted the approval sequence — then audited every export in the submission for provenance.** Req 39 had shipped three CSVs, two of them misnamed (`patching_tickets_oct_2025.csv` held Q4 2025 epic and stories; `patching_tickets_dec_2025.csv` held Q1 2026) with Q2 2026 absent entirely. Rebuilt it as a nine-row monthly approval population plus a seven-row June production patching population, and found two defects in the process. **The first would have read as a control failure:** `aws eks describe-update` returns an offset-aware timestamp the CLI renders in the caller's local zone, and the old collector stripped the offset and stored Pacific wall-clock as UTC — shifting every apply seven hours earlier and putting the AWS apply *before* the GitHub approval that authorised it. Parsed with the offset and converted, five changes show clean approve → merge → apply chains three to fifteen minutes apart. **The second was mine:** I had attributed pull requests to a Jira ticket by matching timing and subject matter, which is inference dressed as a record; the file now carries the branch's own ticket reference or nothing. Also declined to add an `approval_precedes_work_start` verdict column — one month's approval postdates its work-start transition, and a self-scored yes/no would be our reading rather than the record's, so both dates go in raw. Separately confirmed that the EKS upgrade track is a distinct quarterly epic series (SRE-93 / SRE-1559 / SRE-1561) rather than part of monthly patching, and that seven tickets which look like the June change record were all bulk-closed 2026-05-18 against applies that happened 2026-06-26 — citing them would have handed the auditor a resolution date preceding its own change. **Then the question that mattered more than the ticket:** are our exports actually exports? Measured it rather than asserting it — 105 distinct CSVs staged, **91 built by our collectors, 9 genuine vendor exports, and no raw API response staged anywhere**. The tell is header naming: Kandji ships `Device Name, Display OS Version`; our AWS files say `name, state, metric` where the API returns `AlarmName, StateValue, MetricName`. It is a tie-out problem rather than an accuracy problem, but completeness-and-accuracy testing has nowhere to land when the auditor cannot map a column to a source field without reading our code. **Ranked where re-pulls actually matter instead of proposing all 91**, and the ranking found three things the provenance audit hadn't: `vulnerability_open_sample.csv` is exactly 2,000 rows against the 58,752 open vulnerabilities stated in a file sitting in the same folder, and `vulnerability_scan_sample_months.csv` is exactly 50 per month — API page caps presented as samples, with the tie-out number available to the auditor unaided; the CrowdStrike alert population's floor is 2026-06-18, a detection-retention boundary the ~January 2026 migration framing does not explain; and the six CM.02 change populations carry 1,453 rows across two different header schemas with the repo-to-system mapping — the actual completeness claim — appearing nowhere. Everything in the `Config -` folders came off the list: config snapshots don't get completeness-and-accuracy tested, and four artifacts were already correct (Kandji's native export, EL.04's raw workbooks staged beside their populations, Req 96's retention limitation documented with no-data screenshots, and Req 214's disclosed 14-day RDS window with the period-spanning recovery-point inventory as its substitute). Also built a filterable `INDEX.xlsx` over all 339 staged files, since 39 of them are named `IPE_documentation.txt` and Drive search returns them indistinguishably, and moved the tree into Drive for Desktop behind a single resolved path constant rather than the location hardcoded in seventeen collectors. Two self-inflicted errors during the move, both caught by the index reporting files missing and both stated as mine: a manifest folder-path mismatch that appended three duplicate rows, and a nested `CM.09/CM.09/` directory. |
| 2026-09-15 | **Worked the raw re-pull ranking top-down and closed the top three — including a field that would have handed the auditor 98 phantom exceptions on a change-approval control.** Item 1, the vulnerability packet: the two files whose row counts were API page caps presented as samples now state what they are, with exact figures re-pulled from `meta.pagination.total` on `limit=1` queries rather than counted off a truncated page, each named as an extract in the platform's own return order with the population it came from and the full month offered. The IPE's internal contradiction — 400 in the text against a 2,000-row file — is gone. Item 2 turned out to be a wrong premise of mine, and finding that out was worth more than the re-pull would have been: I had ranked the CrowdStrike population's 2026-06-18 floor as detection retention that the migration-maturity framing didn't explain, and told Adam to measure and disclose it. **Falcon carries two date fields that answer opposite questions.** `timestamp` is event time and is *refreshed* on container posture findings, because posture reports present state — filter retention on it and the entire tenant looks ninety days old, including 2.88M container findings that plainly did not all occur in one week. `created_timestamp` is when the record was first raised and is not rewritten; the oldest record in the tenant is a posture finding first raised 2025-12-08 whose `timestamp` reads 2026-07-03, later than its own creation. Retention reaches roughly six months further back than the activity population begins, so retention cannot be what sets that start date, and activity alerts first raised before 2026-06-01 number **0**. The maturity framing was right and my ranking was wrong; the deliverable was the proof (`prove_crowdstrike_retention.py` → ESEC-212), not a re-pull. **Item 3, the six CM.02 change populations, is where retaining raw paid for itself.** All six IPEs stated the path as GitHub API → gh CLI → JSON → CSV, and the JSON step was real but had never been kept, so the CSV was the earliest artifact anyone could inspect. Re-pulling that intermediate through the same `gh` account the IPEs name cost one script and surfaced four defects reading the CSVs could not. **The Servicing population was short ten changes inside its own stated scope** — its IPE claimed 14 repositories and named 13; querying every repository on the mapping workbook's Servicing sheet found `feed-ingestor` (9 merged PRs) and `nd-validations` (1) had never been queried at all. Reissued at 515. MMAX had the same shape of gap (18 named, "+ others", 21 claimed) and its missing three are unrecoverable from any script, so after sweeping 759 org repositories across adjacent naming families and confirming all five candidates out of scope, its IPE now names the 18 actually queried and drops the unsupportable 21. **Three different stated end dates existed for one control and one period** — 2026-09-04, 2026-08-31, and 2026-09-30 on a population collected before that date; proved by re-pull that a single window reproduces all six delivered counts exactly before rewriting anything, so the three reconcile to one without moving a row. **Two files named `*_excluded_out_of_period.csv` held 15 changes merged September 1–4, 2026, which are inside a period running to September 30** — the label asserted Earnest had excluded in-period changes; renamed to the explicit date range with a note stating both boundaries, and staged and manifested for the first time, having existed only as Jira attachments. **The consequential one: `reviewDecision` is current state, not an approval record.** It reverts to `REVIEW_REQUIRED` the moment a commit lands after an approval and stays there after merge, and is null where the base branch has no required-review rule. Taken at face value, 98 of 1,462 merged pull requests read as something other than approved; pulling the actual review history showed **97 of 101 had a peer approval at or before merge**. Carrying that column would have manufactured ~98 exceptions on the most-sampled control in the engagement. Request 27 asks for a population, so the field isn't carried and approval is tested at each pull request URL where the real history lives; the four genuine ones are held internally with the merge actor and event history named — three Copilot agent PRs each merged by a named engineer, one human PR whose ten review events were recorded as `COMMENTED` rather than `APPROVED`. Also normalised all six to one schema against Files.com's separate one, stated the row order, restored full titles from ~80-character truncation, and fixed the consolidated total of **1,441 that was arithmetically wrong on its own terms** — it summed five populations and silently dropped Files.com's 11 — to 1,462 rows / 1,311 unique, the gap being two repositories that each serve two products, stated so the six aren't read as additive. Every IPE is now generated from the JSON beside it, so a count cannot drift from its evidence. **Two process defects of my own, both caught by assertion rather than by reading:** a single-attempt pull took HTTP 502 on the three busiest repositories and recorded them as zero, dropping SchoolHub to 64 of 204 and Servicing to 219 of 515 with nothing in the output saying "truncated" — populations that come out small and plausible are the dangerous kind, so a non-zero exit is now fatal and a genuinely quiet repository is recorded as an explicit zero; and a trailing blank line had made Files.com read as 12 rows against a stated 11, which is where an earlier misread of mine originated, so every population script now asserts `lines == records + 1` and stops. 18 files published across six tickets plus the consolidated IPE and the justification doc, 14 superseded attachments deleted only after each replacement was confirmed live at expected size. Final tree: 351 files, 351 manifest rows, 68 requests, 35 controls, every manifest row backed by a file on disk. |
